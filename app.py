...
РЕКВИЗИТЫ ИЗ ФАЙЛОВ:
- КРИТИЧЕСКИ ВАЖНО: ВСЕГДА внимательно изучай ВСЕ прикреплённые файлы (изображения, PDF, DOCX)
- Ищи и ИЗВЛЕКАЙ: ИНН, КПП, ОГРН, р/с, банк, БИК, к/с, юридический адрес, название организации, ФИО директора
- Файлы могут быть в разных форматах: изображения (JPG, PNG), PDF, DOCX
- ВНИМАТЕЛЬНО читай весь текст в файлах
- Если файл содержит реквизиты → ОБЯЗАТЕЛЬНО используй ИХ
- НЕ ВЫДУМЫВАЙ реквизиты - только из файлов или из текста пользователя
- Если в файле нет реквизитов → оставь [Реквизиты], но НЕ ЗАПРАШИВАЙ у пользователя если файл прикреплён

ФАМИЛИЯ И.О. ИСПОЛНИТЕЛЯ:
- Ищи ФИО директора/руководителя в РЕКВИЗИТАХ из файлов
- Обычно написано: "Директор: ...", "Генеральный директор: ...", "Руководитель: ..."
- Это ФИО которое указано в реквизитах организации
- Если нашёл в файле → используй
- Если в файле нет или файл не прикреплён → оставь [Фамилия И.О.], но НЕ ЗАПРАШИВАЙ если файл есть
    ...
    try:
        ...
        for i, file_data in enumerate(files_data):
            ...
            if 'image' in file_type:
                ...
                # DOCX/DOC файлы - извлекаем текст
                try:
                    file_bytes = base64.b64decode(file_data.get('data', ''))
                    doc = Document(BytesIO(file_bytes))
                    text_content = '\n'.join([paragraph.text for paragraph in doc.paragraphs if paragraph.text.strip()])
                    content.append({'type': 'text', 'text': f"СОДЕРЖИМОЕ DOCX:\n{text_content}"})
                    logger.info(f"✅ DOCX файл {file_name} успешно прочитан, извлечено {len(text_content)} символов")
                except Exception as e:
                    logger.warning(f"Не удалось прочитать DOCX {file_name}: {e}")
            elif any(ext in file_type for ext in ['text', 'csv']):
                try:
...
{'='*80}
КРИТИЧЕСКИ ВАЖНО - ПРИОРИТЕТ ДАННЫХ:
{'='*80}
1. СНАЧАЛА изучи ВСЕ прикреплённые файлы ({len(files_data)} шт.)
2. Извлеки из них: реквизиты, ФИО, адреса, печати
3. ПОТОМ используй текст пользователя
4. ПОТОМ справочники
5. НЕ ЗАПРАШИВАЙ данные которые МОГУТ БЫТЬ в файлах!

Если файлы прикреплены → ОБЯЗАТЕЛЬНО извлеки из них максимум данных!
Запрашивай только то, чего точно нет ни в файлах, ни в тексте.
